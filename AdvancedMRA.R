...
#
#	  R software used to write the 2nd article about MRA : "Unified approach in MRA. Breakthroughs integrated in a new package: MRARegress".
...
#
#	Article : "Unified approach in MRA. Breakthroughs integrated in a new package: MRARegress"
#
...
#								Frank_TFxx_TAyy_z_R1 or R2.rda	: Data generated by Frank and processed (addition of noise level 0.1 or 0.5 times the average concentration of genes),
...
#		./tests/testthat		The unit test files used to check the different code modules (coverage ratio 94%).
...
#	Figure 5a	: 3 kinases network (Thomaseth et al, 2018)						-- Run the code for Table 1 first
...
#	Figure 5b	: Linear 3 genes network (Alon, 2006)							-- Run the code for Table 2 first
Trash	<-	DrawGraph (Res_3L)

#	Figure 5c	: 4 genes network (Eduardo Sontag et al, 2004)					-- Run the code for SI § 2.3 first
Trash	<-	DrawGraph (Res_4G)

#	Figure 5d	: MAPK cascade: 6 nodes network (B. N. Kholodenko et al, 2002)	-- Run the code for SI § 2.4 first
...
#
#	Figure 6
#	Distance to the diagonal of the networks from the "Dream Challenge 4" based on the percentage of known data
#
...
save(Results, file=paste(vRootFig, "Fig6a.rda", sep=""))
save(Res.df,  file=paste(vRootFig, "Fig6a_df.rda", sep=""))
save(Result1G, file=paste(vRootFig, "Fig6a_Result1G.rda", sep="")) 	# To allow for further simulations, if necessary
...
save(Results, file=paste(vRootFig, "Fig6b.rda", sep=""))
save(Res.df,  file=paste(vRootFig, "Fig6b_df.rda", sep=""))
save(Result1G, file=paste(vRootFig, "Fig6b_Result1G.rda", sep="")) 	# To allow for further simulations, if necessary
...
load (paste(vRootFig, "Fig6a.rda", sep=""))			#	Run the code for Fig 6a first
print (Results)
load (paste(vRootFig, "Fig6b.rda", sep=""))			#	Run the code for Fig 6b first
print (Results)
...
save(Results, file=paste(vRootFig, "Fig7.rda", sep=""))
save(Res.df,  file=paste(vRootFig, "Fig7_df.rda", sep=""))
save(Result1G, file=paste(vRootFig, "Fig7_Result1G.rda", sep="")) 	# To allow for further simulations, if necessary
...
NomFic 		<- paste(vRootFig, "Fig7a.pdf", sep="")
...
#
#	Figure 7b	(k=0.5 : strong noise  -- Gaussian noise N(0,sd), with sd = 0.5* mean(abs(MatR))
#

NomFic 		<- paste(vRootFig, "Fig7b.pdf", sep="")
...
#
#	Figure 8
#	Average distances between solutions and calculated matrices, FRANK generated networks (60, 100, 200 nodes, including 50% of non-regulating genes)
...
save(Results, file=paste(vRootFig, "Fig8.rda", sep=""))
save(Res.df,  file=paste(vRootFig, "Fig8_df.rda", sep=""))
save(Result1G, file=paste(vRootFig, "Fig8_Result1G.rda", sep="")) 	# To allow for further simulations, if necessary
...
#
NomFic 		<- paste(vRootFig, "Fig8a.pdf", sep="")
Res1.df		<- subset(Res.df, Noise == 1)
...
#	Now, open "Fig8a.pdf" with Inkscape, arrange the image (font size ...) and export it as . png 600 dpi


#
#	Figure 8b	(k=0.5 : strong noise  -- Gaussian noise N(0,sd), with sd = 0.5* mean(abs(MatR))
#
NomFic 		<- paste(vRootFig, "Fig8b.pdf", sep="")
Res2.df		<- subset(Res.df, Noise == 2)
...
#	Now, open "Fig8b.pdf" with Inkscape, arrange the image (font size ...) and export it as . png 600 dpi
